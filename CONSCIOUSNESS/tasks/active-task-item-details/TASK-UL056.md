# TASK-UL056: Verify lesson 20: independent review and close FEAT-UL18

## Context
Verification task for Part II lesson 20 (Matrix Factorisation Family: NMF and ICA). Notebooks: `notebooks/20a_nmf_ica_theory.ipynb`, `notebooks/20b_nmf_ica_practical.ipynb`. Feature card: `CONSCIOUSNESS/features/backlog-feature-item-details/FEAT-UL18.md`. Story: STORY-UL18. Directive: DIRECT-UL17. This task is the review gate: the feature is kano `performance` (agent gate, one independent review), so an approved verdict here closes the feature, story and directive with no human step.
...
- [ ] Independent review performed by a fresh-context opus subagent against every criterion on FEAT-UL18, with per-criterion evidence recorded on the card
- [ ] Verdict fragment appended (agent-approved or agent-rejected) with the evidence summary
- [ ] On approval: FEAT-UL18 moved to maintained, STORY-UL18 to fulfilled, DIRECT-UL17 to done — indexes and cards both moved
...
## Dependencies
- Blocked by: TASK-UL054, TASK-UL055
