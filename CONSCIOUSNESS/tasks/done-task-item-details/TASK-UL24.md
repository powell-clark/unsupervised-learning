...
## Acceptance Criteria
- [x] MiniSom applied end to end on a real (non-toy-synthetic) dataset
- [x] U-Matrix construction and interpretation: average distance to grid-neighbors per neuron
- [x] U-Matrix visualization revealing cluster boundaries as high-distance "walls"
- [x] Component planes: per-feature heatmaps across the trained grid
- [x] Comparing SOM-derived clusters against a known clustering method (e.g. K-Means) on the same data
- [x] Topology preservation check: quantifying how well grid distance correlates with input-space distance
