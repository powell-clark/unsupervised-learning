...
- [ ] **AC-11** — Cross-lesson link: lessons 8, 9, 10 data generators reused rather than re-invented — NOT MET as literally worded. Lesson 9A's rules and lesson 10B's topic_vocab are genuinely reused (byte-identical, confirmed by the reviewer diffing the literals), but lesson 8's data generator is not reused anywhere and reviewer judges this unavoidable: 8b's data is loaded over the network (forbidden by AC-10), and 8a's own ratings are uniform random with no latent structure worth reusing. Reviewer recommendation, and this card's disposition: a criterion-wording fix (TASK-UL084-style follow-up), not notebook work — tracked as a fresh backlog task per the review-gates rejected-path procedure.
